...
def DDT(sbox):
	length = len(sbox)
	ddt_table = [ [0] * length for _ in range(length) ] #Creating a 2d array of the size of SBox
	for i in range(length):
		for j in range(length):
			inp_diff = i ^ j #Finding Input difference
			out_diff = sbox[i] ^ sbox[j] #Finding output difference
			ddt_table[inp_diff][out_diff] += 1 #Increasing the value of cell corresponding to input_diff and out_diff
