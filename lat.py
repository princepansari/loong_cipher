...
def mask(mask_bit, x):
    ...
    while mask_bit > 0 and x > 0:
        # Product of ith bit  of mask and ith bit of x
        temp = int(mask_bit % 2) * int(x % 2)
        # xor with the product of same ith bit of mask and ith bit of x
...
sbox = [12, 10, 13, 3, 14, 11, 15, 7, 9, 8, 1, 5,
        0, 2, 4, 6]  # SBOX from previos assignment
lat = [[0] * 16 for _ in range(16)]  # 2-D array for Linear Approximation Table

for alpha in range(16):
    for beta in range(16):
        # For each pair (alpha and beta) ranging from 0-15 calculate its biasing and store
